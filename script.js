...
function kochSnowflake() {
	clearBoard();
	const iterations = parseInt(document.getElementById("koch-iter").value, 10) || 4;
	...
	for (let i = 0; i < 3; i++) {
		...
		drawKochCurve(start, end, iterations);
	}
}

function drawKochCurve(start, end, depth) {
	if (depth === 0) {
		ctx.beginPath();
	...
	// Calculate the peak point (p2) using 60° rotation
	const angle = Math.PI / 3; // 60 degrees
	const p2 = {
		x: p1.x + (p3.x - p1.x) * Math.cos(angle) - (p3.y - p1.y) * Math.sin(angle),
		y: p1.y + (p3.x - p1.x) * Math.sin(angle) + (p3.y - p1.y) * Math.cos(angle),
	};
